...
uint8_t PlayfieldData[TETRIS_HEIGHT * ((TETRIS_WIDTH + 7) / 8) * _3BIT];
uint8_t PlayfieldMask[TETRIS_HEIGHT * ((TETRIS_WIDTH + 7) / 8) * _1BIT];
uint8_t CompletedLinesData[TETRIS_SPR_HEIGHT * ((TETRIS_WIDTH + 7) / 8) * _1BIT];
...
void setup()
{
  ...
  Playfield.Setup(TETRIS_WIDTH, TETRIS_HEIGHT, PlayfieldData, 1, _3BIT, TetrisColours, PlayfieldMask);
  ...
  CompletedLines.Setup(TETRIS_WIDTH, TETRIS_SPR_HEIGHT, CompletedLinesData, 1, _1BIT, CompletedLinesColour, CompletedLinesData);
      ...
      else
      {
          ...
          if ( currentInput == ROTATE )
          {
            ...
            if ((CurrentBlock.GetCurrentFrame() % 2) == 1)
            {
              if (CurrentBlock.GetXChange() == 0)
                CurrentBlock.m_X = _min(CurrentBlock.m_X, TETRIS_WIDTH - TETRIS_SPR_WIDTH);
        ...
        if (NextBlock)  // Start new block
        {
          if (CurrentBlock.GetXChange() >= 0) // We have a current block so add to playfield before creating new block
          {
            ...
            uint8_t *Mask = PlayfieldMask;
            uint16_t Mbpl = (TETRIS_WIDTH + 7) / 8;
            int16_t j, numlines = 0;
            for (int16_t i=(TETRIS_HEIGHT-1)*Mbpl, y=0; i>=0; i-=Mbpl,++y)
            {
              for (j=0; j<TETRIS_WIDTH; j+=8)
              {
                if ((uint8_t)(0xff00 >> _min(TETRIS_WIDTH - j, 8)) != Mask[i + (j / 8)])
                  break;
              }
              if (j >= TETRIS_WIDTH)
              {
          ...
          CurrentBlock.SetPositionFrameMotionOptions((TETRIS_WIDTH/2)-1, TETRIS_HEIGHT, 0, 0, 0, 0, -1, DropDelay, SPRITE_DETECT_COLLISION | SPRITE_DETECT_EDGE);
