...
void setup() {
  Serial.begin(115200);
  Serial.println("AnimatedGIF initializing");

  SPI.begin(SD_SCLK, SD_MISO, SD_MOSI, SD_CS);

  // Start filesystem
  Serial.println(" * Opening SDCard");
  if(!SD.begin(SD_CS)) {
    Serial.println("SDCARD init failed");
    while(1);
  }
  uint32_t cardSize = SD.cardSize() / (1024 * 1024);
  String s = "SDCard Size: " + String(cardSize) + " MB";
  Serial.println(s);
...
void showFile(File file) {
  String s = "Showing file " + String(file.name());
  Serial.println(s);
  ...
  //-- Show the current file for max. 8 seconds
  dma_display.fillScreen(dma_display.color565(0, 0, 0));

  long ets = millis() + 8000;
  for(;;) {
    ShowGIF(fname);
...
/**
 * Loop through all files in the dir and show each one. Ignore dirs.
*/
void showDirectory(File root) {
  while(1) {
