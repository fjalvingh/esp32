class Pixel{
  ...
  void setupAlien() {
    ...
    int w = (leds.Width() - 16) / 2;
    int h = (leds.Height() - 16) / 2;
    sprAlien.SetPosition(w,h);
  ...
  void squares() {
    int x2 = leds.Width() / 2;
    int y2 = leds.Height() / 2;

    for (uint8_t i = 0; i < x2; i++) {
      leds.DrawRectangle(i     , i     , x2 - i - 1          , x2 - i - 1           , CHSV(rectHue + (i*15),255,255));
      leds.DrawRectangle(i + x2, i     , leds.Width() - i - 1, x2 - i - 1           , CHSV(rectHue + (i*15),255,255));
      leds.DrawRectangle(i     , i + x2, x2 - i - 1          , leds.Height() - i - 1, CHSV(rectHue + (i*15),255,255));
      leds.DrawRectangle(i + x2, i + x2, leds.Width() - i - 1, leds.Height() - i - 1, CHSV(rectHue + (i*15),255,255));
    }
